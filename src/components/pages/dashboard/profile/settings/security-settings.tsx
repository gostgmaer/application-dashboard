  ...
  export function SecuritySettings() {
    const { user,refetch, loading, updateUser } = useUserData();
    ...
    const confirmTOTP = async () => {
      ...
      try {
        const response = await userApi.confirmTOTP(totpCode);
        if (response.success) {
    ...
    const disableTOTP = async () => {
      ...
      try {
        const response = await userApi.disableTOTP("password");
        if (response.success) {
    ...
    const setupSmsOtp = async (data: PhoneForm) => {
      ...
      try {
        const response = await authService.setupMFA({method: 'sms'},session?.accessToken);
        if (response.success) {
    ...
    const disableSmsOtp = async () => {
      ...
      try {
        const response = await authService.disableMFA({method: 'sms'}, session?.accessToken);
        if (response.success) {
    ...
    const enableEmailOtp = async () => {
      ...
      try {
      const response = await authService.setupMFA({method: 'email'},session?.accessToken);
        if (response.success) {
    ...
    const disableEmailOtp = async () => {
      ...
      try {
        const response = await authService.disableMFA({method: 'email'}, session?.accessToken);
        if (response.success) {
    ...
    const verifyOtp = async (data: OTPVerificationForm) => {
      ...
      try {
        const response = await authService.verifyMFASetup({token:data.otp,method:currentOtpType},session?.accessToken);
        if (response.success) {
          const updateData = currentOtpType === 'sms'
            ? { smsOtpEnabled: true }
          ...
          otpForm.reset();
          toast.success(`${currentOtpType?.toUpperCase()} OTP enabled successfully`);
        }
      } catch (error: any) {
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      copyToClipboard(totpSetup.secret)
                                    }
                                  >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Receive OTP codes at {user.email}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
