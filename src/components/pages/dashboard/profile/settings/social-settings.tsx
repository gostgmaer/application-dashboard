...
import { Share2, Link, Unlink, CircleCheck as CheckCircle, Loader as Loader2, Github, Twitter, Facebook, Mail } from 'lucide-react';
      ...
      <Card className="border-0 shadow-sm bg-white/70 dark:bg-slate-900/70 backdrop-blur-sm">
        ...
        <CardContent>
          <div className="space-y-4">
            {Object.entries(socialProviders).map(([providerId, provider], index) => {
                        ...
                        {isConnected && connection?.email && (
                          <p className="text-xs text-muted-foreground mt-1">
                            ...
                            {connection.connectedAt && (
                              <> • Connected {format(new Date(connection.connectedAt), 'MMM dd, yyyy')}</>
                            )}
                          </p>
                        )}
                      </div>
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button
                              variant="destructive"
                              size="sm"
                              disabled={actionLoading === `disconnect-${providerId}`}
                            >
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This will remove {provider.name} as a sign-in option for your account. 
